...
class OpenAILLM(BaseLLM):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # API key: explicit > settings > profile-specific env var > default env var
        api_key_env = getattr(settings.llm, "api_key_env", None)
        self.api_key = (
            api_key
        ...
        if not self.api_key:
            env_hint = api_key_env or "OPENAI_API_KEY"
            raise ValueError(
                "OpenAI API key not provided. Set in settings.yaml (llm.api_key), "
                f"{env_hint} environment variable, or pass api_key parameter."
            )
        ...
        if base_url or configured_base_url:
