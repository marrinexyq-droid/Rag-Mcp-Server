...
class LLMSettings:
    ...
    # Provider endpoint override (OpenAI-compatible relays, Ollama, etc.)
...
class Settings:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Settings":
        ...
        llm_profile = None
        if "profiles" in llm:
            profiles = _require_mapping(llm, "profiles", "llm")
            llm_profile = _require_str(llm, "active_profile", "llm")
            selected_profile = _require_mapping(profiles, llm_profile, "llm.profiles")
            defaults = {
                key: value
                for key, value in llm.items()
                if key not in {"active_profile", "profiles"}
            }
            llm = {**defaults, **selected_profile}
